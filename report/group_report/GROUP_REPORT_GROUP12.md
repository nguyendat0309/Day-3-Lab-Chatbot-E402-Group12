...
| :--- | :--- | :--- |
| `search_restaurants` | `JSON/kwargs` | Search for a list of restaurants based on filter criteria (cuisine, district, price, rating). |
| `get_restaurant_details` | `string` | Look up detailed information of a specific restaurant based on its name. |
| `check_open_status` | `JSON/kwargs` | Check if a restaurant is currently open at a specific time. |
| `calculate_estimated_cost` | `JSON/kwargs` | Calculate the estimated total cost for a specific number of people eating at a certain restaurant. |
| `human_escalation_fallback` | `string` | Forward the request to human support when the Agent encounters errors or edge cases. |
